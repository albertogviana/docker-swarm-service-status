...
func (s *ServerTestSuite) Test_DeploymentStatus_ReturnSuccess() {
	...
	deploymentStatusMock := service.DeploymentStatus{
	...
	deploymentStatusHandler(router, server)

	imageByte := base64.URLEncoding.EncodeToString([]byte(image))

	rec := httptest.NewRecorder()
	req, _ := http.NewRequest("GET", fmt.Sprintf("/v1/docker-swarm-deployment-status/%s/%s", serviceName, imageByte), nil)
...
func (s *ServerTestSuite) Test_DeploymentStatus_InvalidBase64Parameter() {
	...
	rec := httptest.NewRecorder()
	req, _ := http.NewRequest("GET", fmt.Sprintf("/v1/docker-swarm-deployment-status/%s/%s", serviceName, image), nil)
...
func (s *ServerTestSuite) Test_DeploymentStatus_ReturnError() {
	...
	serviceMock.On("GetDeploymentStatus", serviceName, image).Return(service.DeploymentStatus{}, errors.New("Not able to connect on unix:///var/run/docker.sock"))
	server := &Server{
		serviceMock,
	}
...
func (s *ServiceMock) GetDeploymentStatus(serviceName string, image string) (service.DeploymentStatus, error) {
	args := s.Called(serviceName, image)
	return args.Get(0).(service.DeploymentStatus), args.Error(1)
}
